    ...
    let partitionTest (arr:array<int>) (left: uint) (length:uint) pivot =
      ...
      then
          let (left:int<memoryIndex>), length =
              ...
              let length' =
                  if arr.Length - int left' = 0
                  then 0u
                  else length % uint (abs (arr.Length - int left'))
              int left' * 1<memoryIndex>, int length'

          let expectedLeft, expectedRight = Array.partition (fun x -> x <= pivot) arr.[int left..(int left + length - 1)]
          let actualLeft, actualRight =
              let tmp = Array.zeroCreate arr.Length
              let left, right = partition
                                    (MyArray(arr, left, length))
                                    (MyArray(tmp, left, length))
                                    pivot
